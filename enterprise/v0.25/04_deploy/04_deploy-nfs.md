...
- You can configure an NFS volume only for Airflow Deployments running Airflow 2.0+.
- You can only use NFS volumes for deploying DAGs. To push dependencies or other requirements to your Airflow Deployment, you still need to set update your `requirements.txt` and `packages.txt` files and rebuild your image via the CLI as described in [Customize Images](/docs/enterprise/v0.25/develop/customize-image).
