...
### Access dashboards for specific Airflow Deployments
...
3. In Grafana, open a dashboard. Click the menu under the Deployment's name to see a list of all possible release names. Depending on which dashboard you're using, the name of this menu is either **Deployment** or **Release Name**.

    ![Release name menu in Grafana](https://assets2.astronomer.io/main/docs/grafana/select-release-name.png)

4. Select one or multiple release names to monitor in the dashboard.

Read the rest of this guide to learn about Astronomer's built-in dashboards, as well as key metrics for tracking the overall health of your platform.
...
This dashboard tracks the performance of Fluentd, which is the primary logging tool for Astronomer. You can use the metrics in this dashboard to track the overall status of logs being emitted from the service.
...
In addition to their dashboards in Grafana, each of your Deployments has a built-in metrics dashboard in the Astronomer UI. To get there, open a Deployment and go to the **Metrics** tab:
