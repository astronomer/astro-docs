...
| Flag                | Value Type | Usage                                                                             |
| ------------------- | ---------- | --------------------------------------------------------------------------------- |
| `--airflow-version` | String     | The version of Airflow you want to use. The default value is the latest version available.  |
| `--name`            | String     | The name for the Airflow project.                                                 |

## astro dev kill

Kills the Webserver, Scheduler, and Postgres Docker containers and removes all local volumes first created with `$ astro dev start`. This command will delete all data associated with your local Postgres Metadata Database, including Airflow Connections, logs, and task history.
...
Runs a single [Airflow CLI command](https://airflow.apache.org/docs/apache-airflow/stable/cli-ref.html) on your local Airflow environment. This command only applies to local development and is not supported on Astronomer.
