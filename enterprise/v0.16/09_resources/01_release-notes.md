...
Astronomer v0.16.6 allows users to define custom alerts via the `config.yaml` file and successfully pass those to Prometheus, a monitoring tool that Astronomer uses to collect metrics from StatsD and push them to Grafana for visualization.

For example, you might want to add an Airflow alert to fire if more than 10% of tasks across Airflow Deployments are failing within a 30 minute timespan or a Platform alert if more than 2 Airflow Schedulers don't heartbeat for more than 5 minutes. Here's what your `config.yaml` might look like:
    ...
    additionalAlerts:
        airflow:
          - alert: ExampleAirflowAlert
            ...
            expr: 100 * sum(increase(airflow_ti_failures[30m])) / (sum(increase(airflow_ti_successes[30m]))) > 10
            for: 15m
            labels:
                tier: airflow
            annotations:
