...
> **Note:** The guidelines below assume that you've initialized a project on Astronomer via `$ astro dev init`. If you haven't done so already, refer to our ["CLI Quickstart" doc](/docs/cloud/stable/develop/cli-quickstart/).
...
> **Note:** The Astronomer CLI does _not_ currently support overrides to Environment Variables. For more information on how to set, configure and customize those values, refer to our ["Environment Variables" doc](/docs/cloud/stable/deploy/environment-variables/).
...
>
> For more detail on how to add Environment Variables both locally and on Astronomer, refer to our [Environment Variables doc](/docs/cloud/stable/deploy/environment-variables/).
...
If you're running the Astronomer Core 2.0.0 image, this would be:

```
FROM quay.io/astronomer/core:2.0.0 AS stage1
```
...
If you have `quay.io/astronomer/core:2.0.0-buster` in your `Dockerfile.build`, for example, this command would be:

```
$ docker build -f Dockerfile.build --build-arg PRIVATE_RSA_KEY="$(cat ~/.ssh/id_rsa)" -t custom-core:2.0.0-buster .
```
...
If you're running `quay.io/astronomer/core:2.0.0-buster` as specified above, this line would read:

```
FROM custom-core:2.0.0-buster
```
