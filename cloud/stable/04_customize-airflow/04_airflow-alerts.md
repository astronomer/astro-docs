...
| `AirflowEphemeralStorageLimit` | The Airflow Deployment has been using more than 5GB of its ephemeral storage for over 10 minutes. | Make sure to continually remove unused temporary data in your Airflow tasks. |
| `AirflowPodQuota` | The Airflow Deployment has been using over 95% of its pod quota for over 10 minutes. | Either increase the Deployment's Extra Capacity in the Astronomer UI or update your DAGs to use less resources. If you have not already done so, upgrade to [Airflow 2.0](https://www.astronomer.io/blog/introducing-airflow-2-0) for improved resource management. |
...
| `AirflowTasksPendingIncreasing` | The Airflow Deployment created tasks faster than it was clearing them for over 30 minutes. | Ensure that your tasks are running and completing correctly. If your tasks are running as expected, [maximize Airflow's concurrency and parallelism](https://www.astronomer.io/guides/airflow-scaling-workers) and increase the Deployment's allocated resources accordingly so that fewer tasks are concurrently pending. |
| `ContainerMemoryNearTheLimitInDeployment` | A container in your Airflow Deployment is near its memory quota; it's been using over 95% of its memory quota for over 60 minutes. | Either increase the Deployment's allocated resources in the Astronomer UI or update your DAGs to use less resources. If you have not already done so, upgrade to [Airflow 2.0](https://www.astronomer.io/blog/introducing-airflow-2-0) for improved resource management. |
