...
| `AirflowPodQuota` | The Airflow Deployment is near its pod quota; it's been using over 95% of it's pod quota for over 10 minutes. | Either increase the Deployment's Extra Capacity in the Astronomer UI or update your DAGs to use less resources. If you have not already done so, upgrade to [Airflow 2.0](https://www.astronomer.io/blog/introducing-airflow-2-0) for improved resource management. |
...
| `AirflowTasksPendingIncreasing` | The Airflow Deployment is creating tasks faster than it's clearing them. | Ensure that your tasks are running and completing correctly. If your tasks are running as expected, increase the Deployment's allocated resources in the Astronomer UI or redesign your DAGs so that fewer tasks are concurrently pending. |
| `ContainerMemoryNearTheLimitInDeployment` | A container in your Aiirflow Deployment is near its memory quota; it's been using over 95% of it's memory quota for over 60 minutes. | Either increase the Deployment's allocated resources in the Astronomer UI or update your DAGs to use less resources. If you have not already done so, upgrade to [Airflow 2.0](https://www.astronomer.io/blog/introducing-airflow-2-0) for improved resource management. |
| `CpuThrottlingInDeployment` | The Airflow Deployment is near its CPU quota; it's been using over 95% of it's CPU quota for over 10 minutes. | Either increase the Deployment's allocated resources in the Astronomer UI or update your DAGs to use less resources. If you have not already done so, upgrade to [Airflow 2.0](https://www.astronomer.io/blog/introducing-airflow-2-0) for improved resource management. |
